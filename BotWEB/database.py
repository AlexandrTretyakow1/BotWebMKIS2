...
async def ProcessSearchModel(message):  #Функция добавления ключевых слов
    search_exist = True
    try:
        search = SearchModel.select().where(SearchModel.title == message.text).get() #Пытаемся найти ключевое слово
    ...
    if not search_exist:
        ...
        rec.save()
        await message.answer("строка поиска {} добавленна".format(message.text))
    else:
